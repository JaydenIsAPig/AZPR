...
- Prompt 35 production release-plan commit plus separately signed two-person human-operated production apply evidence bound to that exact plan and commit.
...
The roadmap must declare Prompt 035 as the prerequisite evidence-producing stage and must never contain a mutable evidence ID. The controller supplies exactly one verified successful evidence envelope attached to an open Prompt 035 registration for this audit. Missing, ambiguous, replayed, stale, failed, partial or rolled-back evidence is BLOCKED.
