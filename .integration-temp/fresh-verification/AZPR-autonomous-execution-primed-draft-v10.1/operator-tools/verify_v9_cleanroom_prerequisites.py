#!/usr/bin/env python3
"""Non-root v9 semantic qualification verifier.
...
from v9_evidence_derivation import verify_report_derivation  # noqa: E402
from v9_external_signer_client import request_external_signature  # noqa: E402
from v9_probe_boundary import TARGETS, verify_probe_manifest  # noqa: E402
...
ARTIFACT_KINDS = {
    ...
    "approval_keyring": "json", "evidence_keyring": "json",
    ...
    "agent_execution_profile": "json", "handoff_quota_profile": "json",
    "key_lifecycle_state": "json",
    **{name: "qualification_report" for name in REPORT_ROLES},
}
assert len(ARTIFACT_KINDS) == 39
...
def inspect_artifacts(rows: dict[str, Any]) -> tuple[dict[str, dict[str, Any]], dict[tuple[int, int], str]]:
    if set(rows) != set(ARTIFACT_KINDS):
        raise SystemExit(f"qualification artifact set mismatch: missing={sorted(set(ARTIFACT_KINDS)-set(rows))} extra={sorted(set(rows)-set(ARTIFACT_KINDS))}")
    results: dict[str, dict[str, Any]] = {}
    identities: dict[tuple[int, int], str] = {}
    for name in sorted(ARTIFACT_KINDS):
        row = rows[name]
...
def build_unsigned_receipt(value: dict[str, Any], results: dict[str, dict[str, Any]], report_hashes: dict[str, str]) -> dict[str, Any]:
    issued = datetime.now(timezone.utc)
    bindings = {name: results[name]["sha256"] for name in sorted(results)}
    return {
        "format_version": "3.0",
        "receipt_kind": "AZPR_V9_SEMANTIC_CLEANROOM_QUALIFICATION_RECEIPT",
        "receipt_id": "qual-receipt-" + os.urandom(16).hex(),
        ...
        "probe_envelopes_manifest_sha256": sha_bytes(read_regular(Path(str(value["probe_envelopes_manifest"])))),
        "external_signer_identity_sha256": value["external_signer_identity_sha256"],
        ...
        "signer_key_id": value["receipt_signing_key_id"],
    }


def verify_inputs_and_build(value: dict[str, Any]) -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, str]]:
    expected_top = {
        "format_version", "safe_for_unattended_execution_now", "trusted_pre_autonomous_installation_ready",
        "candidate_archive_sha256", "bundle_manifest_sha256", "host_id", "agent_uid", "agent_gid",
        "validation_image_digest", "receipt_signing_key_id", "external_signer_client",
        "external_signer_identity_sha256", "probe_keyring", "probe_envelopes_manifest", "raw_evidence", "probe_targets", "artifacts",
    }
    if set(value) != expected_top or value.get("format_version") != "1.0":
        raise SystemExit("v9 qualification input document shape/version mismatch")
    if value.get("safe_for_unattended_execution_now") is not False or value.get("trusted_pre_autonomous_installation_ready") is not False:
        raise SystemExit("v9 qualification input readiness flags must remain false")
    if not HEX64.fullmatch(str(value.get("candidate_archive_sha256"))) or not HEX64.fullmatch(str(value.get("bundle_manifest_sha256"))):
        raise SystemExit("v9 qualification candidate/manifest hash invalid")
    if int(value.get("agent_uid", 0)) <= 0 or int(value.get("agent_gid", 0)) <= 0:
        raise SystemExit("v9 qualification agent identity must be explicit non-root")
    results, _ = inspect_artifacts(value["artifacts"])
    ...
    probe_keyring = load_json(Path(str(value["probe_keyring"])))
    verify_probe_manifest(
        Path(str(value["probe_envelopes_manifest"])),
        target_hashes=target_hashes,
        probe_keyring=probe_keyring,
    )
    ...
    unsigned = build_unsigned_receipt(value, results, report_hashes)
    validate_schema("cleanroom-qualification-receipt-v9.schema.json", {**unsigned, "signature": "A" * 86 + "=="})
    return unsigned, results, report_hashes
...
def main() -> None:
    ...
    if os.geteuid() == 0:
        raise SystemExit("v9 cleanroom verifier refuses root execution")
    value = load_json(Path(args.inputs))
    if any("private" in key.lower() for key in value if key not in {"artifacts"}):
        raise SystemExit("v9 verifier top-level input must not accept receipt private-key material")
    unsigned, results, report_hashes = verify_inputs_and_build(value)
    receipt = request_external_signature(
        unsigned_receipt=unsigned,
        signer_client=Path(str(value["external_signer_client"])),
        signer_identity_sha256=str(value["external_signer_identity_sha256"]),
        receipt_public_key=Path(results["qualification_receipt_signing_public_key"]["path"]),
        request_nonce="sign-request-" + os.urandom(16).hex(),
    )
    validate_schema("cleanroom-qualification-receipt-v9.schema.json", receipt)
