...
def _invoke_anchor(operation: str, payload: dict[str, Any]) -> None:
    ...
    child_env={"PATH":"/usr/bin:/bin","HOME":"/nonexistent","LANG":"C.UTF-8","LC_ALL":"C.UTF-8"}
    # The bundled reference anchor is source-test infrastructure only. Preserve
    # its explicitly test-owned storage root so non-root verification does not
    # fall back to a shared /tmp path. Production mode never accepts this
    # variable and continues to consume only the installed pinned helper.
    if not _ti.production_mode() and helper.name == "file_anchor_reference.py":
        reference_root=os.environ.get("AZPR_REFERENCE_ANCHOR_ROOT")
        if reference_root:
            child_env["AZPR_REFERENCE_ANCHOR_ROOT"]=reference_root
...
def run_bounded_process(
    ...
    try:
        ...
        self_identity_test=bool(not _ti.production_mode() and os.environ.get('AZPR_EXPLICIT_TEST_MODE')=='1' and run_as_uid==os.geteuid() and run_as_gid==os.getegid())
        process=subprocess.Popen(command,cwd=cwd,stdin=subprocess.PIPE if input_bytes is not None else subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.PIPE,env=child_env,start_new_session=True,user=None if (gate_is_privileged or self_identity_test) else run_as_uid,group=None if (gate_is_privileged or self_identity_test) else run_as_gid,extra_groups=None if (gate_is_privileged or self_identity_test) else ([] if run_as_uid is not None else None),pass_fds=pass_fds)
