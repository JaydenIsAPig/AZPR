...
prompt_id: "032"
sequence: 32
title: "Formal Pilot Release Test Matrix and Release Gate Audit"
...
- Prompts 28-31 validated commits.
...
- Create one new immutable report at docs/audits/pilot-release-gate-audit-<YYYY-MM-DD>-<shortsha>.md. Never overwrite a prior audit report.
...
AUDIT OBJECTIVE
Evaluate the complete pilot against a release test matrix and decide whether staging deployment is allowed.

REQUIRED MATRIX
- Domain unit tests; parser fixtures; database/repository integration; migration safety; connector contracts; API; authentication/authorization; customer isolation; UI component/browser/end-to-end onboarding; end-to-end live-source ingestion; normalization/deduplication/classification/geography; Opportunity matching/explainability; dashboard/lead workflow; email; disabled SMS; internal operations; analytics/observability; retry/idempotency/concurrency; accessibility; mobile responsiveness; performance; backup restoration; rollback; security scanning; documentation synchronization.

THE GATE MUST NOT PASS WHEN
- Any critical test fails.
- Required migrations are unvalidated or destructive without approval.
- A source/Permit/Match/notification cannot be traced through all required versions.
- Duplicate notification occurs in the required scenario.
- Customer isolation or anti-enumeration fails.
- Raw artifacts can be mutated or evidence/history can be deleted through normal workflows.
- Low-confidence or unresolved-review records bypass publication rules.
- Stale Opportunity revisions can be shown or sent as current.
- Consent, unsubscribe, or SMS disablement can be bypassed.
- Source access approval/freshness monitoring is absent.
- Backup restore/reconciliation or rollback evidence is absent.
- Secrets or prohibited sensitive data appear in repository/logs/metrics.
- Current documentation contradicts code or deployed configuration.

RESULT
Prompt 33 may run only on PASS. PASS WITH REQUIRED CORRECTIONS must name the exact prompt/stage to rerun; BLOCKED must identify the release blocker.
