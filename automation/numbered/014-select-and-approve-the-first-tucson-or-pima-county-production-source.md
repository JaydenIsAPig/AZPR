...
prompt_id: "014"
sequence: 14
title: "Select and Approve the First Tucson or Pima County Production Source"
stage_group: "S5 - Live Source and Geography"
effort_label: "Ultra"
prompt_type: "Source Access Decision Gate"
...
- Required predecessor evidence: Prompt 13 audit commit.
...
TASK
Select and approve the first production municipal source within the Tucson/Pima County source family while preserving modular support for the other candidate later.

SOURCE-ACCESS DECISION GATE
Use the source registry contract and onboarding checklist. Compare Tucson and Pima County candidate publications/endpoints for:
- terms, automated-access permission, robots/rate limits, authentication, ownership, and operating cost;
- freshness, frequency, history/backfill, identifiers, dates, status, valuation, location/parcel quality, format stability, and pilot-trade value;
- parser/normalization effort, overlap/deduplication, outage/change handling, and ongoing maintenance.

APPROVAL RULE
- If access/compliance, ownership, cost, or technical constraints are uncertain, create a proposed source decision and stop for explicit human approval without live access.
- Codex may finalize a clearly public, approved, low-risk candidate only when every selection criterion and absence of an approval trigger are documented.
- A proposal-only run may create a decision-proposal commit; after approval, rerun for the decision-finalization commit.

REQUIRED OUTPUTS AFTER APPROVAL
- One enabled source profile covering acquisition contract, schedule/time zone, user agent/rate limits, parser/version, expected identifiers/dates, history, limitations, health/access status, owner, and disable switch.
- Keep the other Tucson/Pima candidate disabled/planned; capture lawful immutable fixtures without secrets or unnecessary personal data.
- Keep source mappings outside shared domain logic and document source-change, downtime, stale-publication, backfill, and future overlap/deduplication behavior.

ACCEPTANCE CRITERIA
Exactly one first production source is approved and enabled with documented access basis and operating limits; the other remains modular and disabled, and no live access begins under ambiguous terms.
