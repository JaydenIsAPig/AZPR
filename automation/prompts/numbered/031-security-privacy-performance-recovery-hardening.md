...
- Required predecessor evidence: Prompt 30 validated backup/restore/reconciliation commit.
...
TASK
Perform the formal security/privacy hardening review of the implemented pilot and fix high-confidence critical or high-severity issues that can be corrected without unauthorized architecture expansion.

REQUIRED REVIEW
- Authentication, session/token security, authorization, object-level access, account enumeration, customer isolation, internal/admin access, rate limiting, CSRF, CORS, input validation, injection, XSS, SSRF, file/source ingestion, webhook verification, secret management, dependency vulnerabilities, logging, raw artifact access, notification consent, backups/restores, and deployment configuration.
- Data-flow diagram, threat model, data classification/retention policy, security test plan, incident response outline, and vulnerability register.
- Performance/load tests for representative acquisition, parsing, matching, dashboard queries, outbox/worker, and notification workloads.
- Recovery tests for worker crash, provider outage, database restart, restore, rollback, and accidental configuration change.

FIX AUTHORITY
Fix evidence-supported critical/high issues within the approved stack. Stop for approval before destructive migrations, provider/architecture replacement, material recurring-cost changes, or behavior requiring legal/compliance decisions. Do not make legal compliance claims.

REQUIRED TOOLING
Run configured secret scanning, dependency vulnerability scanning, static analysis, linting, type checks, security-focused tests, and container/infrastructure scans when applicable. Record accepted risks with owner and expiry/review date; do not suppress findings merely to obtain a pass.

ACCEPTANCE CRITERIA
No known unmitigated critical/high vulnerability remains within pilot scope, customer isolation is preserved under adversarial tests, sensitive data is minimized, and recovery/performance evidence is documented.
