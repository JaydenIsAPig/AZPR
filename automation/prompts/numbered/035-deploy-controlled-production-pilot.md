...
- Required predecessor evidence: Prompt 34 audit commit and explicit human authorization for controlled production deployment.
...
TASK
Deploy the narrow AZ Permit Radar vertical slice to a controlled production pilot only after the staging readiness audit returns PASS and the human owner explicitly authorizes production deployment in this run.

AUTHORIZATION GATE
- Without explicit production-deployment authorization, prepare the final checklist and stop before pushing, provisioning, migrating, changing DNS/provider production settings, enabling schedules, or enrolling pilot users.
- If the approved deployment workflow requires a push or external action, perform it only when explicitly authorized and credentials/permissions are available. Never expose credentials in output or logs.

REQUIRED DEPLOYMENT SCOPE
- Deploy exactly the audited commit/configuration for one approved Tucson/Pima source, the approved limited trades, authenticated customer dashboard, email delivery, internal review, and monitoring.
- Apply production migrations through the approved gated process with preflight backup and rollback/forward-fix plan.
- Configure bounded source schedule/backfill, worker concurrency, provider limits, secrets, storage, monitoring, alerts, backups, support escalation, and incident runbooks.
- Enroll only approved pilot users/accounts and verify Customer Account relationships/permissions.
- Keep billing, CRM, additional sources, marketplace behavior, and microservices disabled/out of scope.
- Keep production SMS disabled unless Appendix B and Appendix C have both completed with explicit approval and PASS evidence.
- Use feature flags for AI classification and experimental scoring according to approved policies.
- Execute production-safe smoke tests that do not create duplicate alerts or expose another customer's data.
- Record deployed commit, configuration versions, migrations, source profile, feature flags, provider modes, pilot cohort, support contacts, and rollback point in a deployment evidence report.

ACCEPTANCE CRITERIA
The controlled production environment runs the exact approved narrow pilot with one source family/one enabled source, limited trades, authenticated customer isolation, explainable current Matches, email alerts, internal review, monitoring, backups, and a tested rollback path. No broader capability is activated.
